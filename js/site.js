...
function EditDetails(e, currslider) {
    ...
    switch (detailsTitle) {
        case "Мониторинг аварий":
            detailcontent.innerText = "Мониторинг аварий позволяет в режиме реального времени просматривать аварии на БС. Информация отображается в виде таблицы и может быть отфильтрована по технологиям. Также сверху присутствует строка поиска.";
            break;
        ...
        case "Детальный просмотр статуса аварий":
            detailcontent.innerText = "Позволяет отображать статус работы секторов, тип и серийный номер RRU смонтированных на БС, температуру на TCU, а также адрес БС, информацию по допуску и данные арендодателя на выбранной БС. Также к данной странице программы есть доступ из меню Справочник БС";
            break;
        ...
        case "Закрытие заявок TT в системе Servise Desk":
            ...
            break;
        case "Формирование отчета по закрытым авариям":
            detailcontent.innerText = "Программа позволяет сформировать отчет за выбранный период времени, на выбранных БС, отображающий все аварии, которые были на выбранных БС за выбранный промежуток времени. Отчет формируется в формате CSV ";
            break;
        case "Создание заявки для РТК в системе NOC":
            detailcontent.innerText = "При помощи библиотеки Selenium Web Driver программа позволяет создать и отправить заявку в РТК. Для удобства пользователя реализована возможность выбрать тип заявки (АВР по умолчанию, ППР при выборе соответствующего флага), также в окне создания заявки отображаются все активные аварии на БС и перечень дополнительных работ, если таковые есть";
            break;
        case "Окно авторизации":
            detailcontent.innerText = "Производится авторизация пользователя и в соответствии с его ролью на странице отображаются необходимые данные";
            break;
        case "Создание чек-листа":
            detailcontent.innerText = "На этой странице пользователь с ролью аналитик может создать чек-лист и указать первичные параметры";
            break;
        case "Детальный просмотр чек листа":
            detailcontent.innerText = "На этой странице пользователь может увидеть статус заполнения чек-листа всеми пользователями.";
            break;
        case "Живой поиск":
            detailcontent.innerText = "В WEB-приложении осуществлен живой поиск с фильтрацией по параметрам";
            break;
        case "Обновление данных чек-листа":
            detailcontent.innerText = "Пользователь заполняет данные в чек-листе в соответствии со своей ролью и параметрами выбраными аналитиком";
            break;
        case "Создание чек-листа":
            detailcontent.innerText = "На этой странице пользователь с ролью аналитик может создать чек-лист и указать первичные параметры";
            break;
        case "Описание":
            detailcontent.innerText = "Основное окно Telegram bot-а с описанием";
            break;
        case "Выбор операции":
            detailcontent.innerText = "Можно выбрать отображение аварий или статус работы секторов на выбранной БС, активные аварии POWER, неработающие сектора БС технологии GSM";
            break;
        case "Отображение результата":
            detailcontent.innerText = "Отображение результата";
            break;
        default:
            break;
    }
